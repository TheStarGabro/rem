...
if readFile and writeFile then
    ...
    if not ran or releaseInfo.tag_name ~= result then
        ...
        function environment.import(asset)
            ...
            elseif web then
                if readFile and writeFile then
                    local file = (hasFolderFunctions and "hydroxide/user/" .. user .. '/'..rep..'/' .. asset .. ".lua") or ("hydroxide-" .. user .. '-' .. asset:gsub('/', '-') .. ".lua")
                    local content

                    if (isFile and not isFile(file)) or not importCache[asset] then
                        content = game:HttpGetAsync("https://raw.githubusercontent.com/" .. user .. "/"..rep.."/" .. branch .. '/Hydroxide/' .. asset .. ".lua")
                    ...
                    assets = { loadstring(game:HttpGetAsync("https://raw.githubusercontent.com/" .. user .. "/"..rep.."/" .. branch .. '/Hydroxide/' .. asset .. ".lua"), asset .. '.lua')() }
                end
            else
